...
from config import API_HASH, APP_ID, LOGGER, TG_BOT_TOKEN, TG_BOT_WORKERS, CHANNEL_ID, PORT, CHANNEL_ONE, CHANNEL_TWO, CHANNEL_THREE
...
class Bot(Client):
    ...
    async def start(self):
        ...
        if CHANNEL_THREE:
            try:
                link_a = (await self.create_chat_invite_link(chat_id=CHANNEL_three, creates_join_request=True)).invite_link 
                self.link_three = link_a                                     
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot can't Export Invite link from Force Sub Channel!")
